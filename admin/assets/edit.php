...
$where_asset = "a.id = :id AND a.school_id = :sid";
$asset_params = [':id' => $id, ':sid' => $sid];

if ($assigned_lid && !is_super_admin() && !is_head_teacher()) {
    $where_asset .= " AND a.location_id = :assigned_lid";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $quantity = max(1, (int)v('quantity', $asset['quantity'] ?? 1));
  ...
  $qty_available = max(0, (int)v('qty_available', $asset['qty_available'] ?? 0));
  $qty_in_use = max(0, (int)v('qty_in_use', $asset['qty_in_use'] ?? 0));
  $qty_maintenance = max(0, (int)v('qty_maintenance', $asset['qty_maintenance'] ?? 0));
  $qty_faulty = max(0, (int)v('qty_faulty', $asset['qty_faulty'] ?? 0));
  $qty_lost = max(0, (int)v('qty_lost', $asset['qty_lost'] ?? 0));
  ...
  // Auto-generate asset code if not already set (mostly for existing records switching to non-elec, though unlikely)
  if ($isNonElectronic) {
    if ($asset_code === '') {
    ...
    // Set a primary status for compatibility (highest quantity)
    $breakdown = [
...
<div class="card table-card">
  <div class="card-body">
    <form method="post" enctype="multipart/form-data" class="row g-3">
      <?php
      $isNonElectronicInitial = ($asset['cat_type'] === 'Non-Electronic');
      ?>
      <div class="col-12 col-md-4" id="asset_code_group" style="display:<?php echo $isNonElectronicInitial ? 'none' : 'block'; ?>;">
        <label class="form-label">Asset Code</label>
        <input class="form-control" name="asset_code" value="<?php echo htmlspecialchars(v('asset_code', $asset['asset_code'])); ?>" <?php echo $isNonElectronicInitial ? 'readonly' : ''; ?>>
        <div class="form-text non-elec-hint" style="display:<?php echo $isNonElectronicInitial ? 'block' : 'none'; ?>;">Auto-generated for non-electronic assets.</div>
      </div>
        ...
        <select class="form-select" name="category_id" id="category_select" required>
          ...
          <?php foreach ($categories as $c): ?>
            <option value="<?php echo (int)$c['id']; ?>" 
                    ...
                    <?php echo ((int)v('category_id', $asset['category_id']) === (int)$c['id']) ? 'selected' : ''; ?>>
              <?php echo htmlspecialchars($c['name']); ?>
            </option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="col-12 col-md-4 ict-field" style="display:<?php echo $isNonElectronicInitial ? 'none' : 'block'; ?>;">
      ...
      <div class="col-12 col-md-4" id="quantity_group">
        ...
        <input type="number" class="form-control" name="quantity" min="1" value="<?php echo (int)v('quantity', $asset['quantity']); ?>" id="quantity_input" <?php echo $isNonElectronicInitial ? 'readonly' : ''; ?>>
        <div class="form-text">Number of units.</div>
      </div>

      <!-- Non-Electronic Breakdown -->
      <div class="col-12 non-elec-breakdown" style="display:<?php echo $isNonElectronicInitial ? 'block' : 'none'; ?>;">
        <div class="card bg-light border-0 shadow-none">
          <div class="card-body p-3">
            <h6 class="fw-bold mb-3 small text-uppercase text-primary d-flex align-items-center">
                <i class="bi bi-grid-3x3-gap me-2"></i>Status Breakdown
            </h6>
            <div class="row g-2">
              <div class="col-6 col-md-2">
                <label class="x-small text-secondary text-uppercase fw-bold" style="font-size: 0.65rem;">Working</label>
                <input type="number" class="form-control form-control-sm breakdown-input" name="qty_available" value="<?php echo (int)v('qty_available', $asset['qty_available'] ?? 0); ?>" data-status="Available">
              </div>
              <div class="col-6 col-md-2">
                <label class="x-small text-secondary text-uppercase fw-bold" style="font-size: 0.65rem;">In Use</label>
                <input type="number" class="form-control form-control-sm breakdown-input" name="qty_in_use" value="<?php echo (int)v('qty_in_use', $asset['qty_in_use'] ?? 0); ?>" data-status="In Use">
              </div>
              <div class="col-6 col-md-2">
                <label class="x-small text-secondary text-uppercase fw-bold" style="font-size: 0.65rem;">Maintenance</label>
                <input type="number" class="form-control form-control-sm breakdown-input" name="qty_maintenance" value="<?php echo (int)v('qty_maintenance', $asset['qty_maintenance'] ?? 0); ?>" data-status="Maintenance">
              </div>
              <div class="col-12 col-md-3">
                <label class="x-small text-secondary text-uppercase fw-bold text-danger" style="font-size: 0.65rem;">Damaged / Faulty</label>
                <input type="number" class="form-control form-control-sm breakdown-input" name="qty_faulty" value="<?php echo (int)v('qty_faulty', $asset['qty_faulty'] ?? 0); ?>" data-status="Faulty">
              </div>
              <div class="col-6 col-md-2">
                <label class="x-small text-secondary text-uppercase fw-bold" style="font-size: 0.65rem;">Lost</label>
                <input type="number" class="form-control form-control-sm breakdown-input" name="qty_lost" value="<?php echo (int)v('qty_lost', $asset['qty_lost'] ?? 0); ?>" data-status="Lost">
              </div>
            </div>
            <div class="form-text mt-2 small text-primary italic">Note: Updating these values automatically calculates the Total Quantity.</div>
          </div>
        </div>
      </div>
      <div class="col-12 col-md-4" id="power_adapter_group" style="display:<?php echo $isNonElectronicInitial ? 'none' : 'block'; ?>;">
      ...
      <div class="col-12 col-md-4" id="power_status_group" style="display:<?php echo (!$isNonElectronicInitial && $asset['power_adapter'] === 'Yes') ? 'block' : 'none'; ?>;">
      ...
      <?php 
        $catNameInitial = strtolower($asset['cat_name']);
        $showCableInitial = strpos($catNameInitial, 'projector') !== false || strpos($catNameInitial, 'desktop') !== false || strpos($catNameInitial, 'computer') !== false || strpos($catNameInitial, 'printer') !== false;
      ?>
      <div class="col-12 col-md-4 cable-group" style="display:<?php echo ($showCableInitial && !$isNonElectronicInitial) ? 'block' : 'none'; ?>;">
        <label class="form-label" id="cable_label"><?php echo strpos($catNameInitial, 'printer') !== false ? 'Printing' : 'Display'; ?> Cable?</label>
      ...
      <div class="col-12 col-md-4 cable-group" style="display:<?php echo ($showCableInitial && !$isNonElectronicInitial) ? 'block' : 'none'; ?>;">
        <label class="form-label" id="cable_type_label"><?php echo strpos($catNameInitial, 'printer') !== false ? 'Printing' : 'Display'; ?> Cable Type</label>
      ...
      <div class="col-12 col-md-4 cable-group" style="display:<?php echo ($showCableInitial && !$isNonElectronicInitial) ? 'block' : 'none'; ?>;">
        <label class="form-label" id="cable_status_label"><?php echo strpos($catNameInitial, 'printer') !== false ? 'Printing' : 'Display'; ?> Cable Status</label>
